...
function LeadForm({ compact = false }: { compact?: boolean }) {
  ...
  return submitted ? (
    ...
    <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true) }} className="flex flex-col gap-4">
      ...
      <button type="submit" className="flex h-12 items-center justify-center gap-2 rounded-md bg-accent px-5 text-sm font-semibold text-accent-foreground transition hover:bg-accent-hover">{compact ? 'Get Property Recommendations' : 'Request Property Recommendations'} <ArrowRight size={16} /></button>
    ...
    <main className="min-h-screen bg-background text-foreground">
      <nav className="fixed inset-x-0 top-0 z-50 border-b border-white/15 bg-primary/95 text-primary-foreground backdrop-blur-md">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
          ...
          <button onClick={() => scrollTo('contact')} className="hidden rounded-md bg-accent px-5 py-3 text-xs font-bold text-accent-foreground transition hover:bg-accent-hover sm:block">Book a Site Visit</button>
          <button className="lg:hidden" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation">{menuOpen ? <X /> : <Menu />}</button>
        </div>
        {menuOpen && <div className="flex flex-col gap-5 border-t border-white/10 bg-primary px-5 py-6 lg:hidden"><Link href="/properties" onClick={() => setMenuOpen(false)} className="text-left">Properties</Link><Link href="/locations" onClick={() => setMenuOpen(false)} className="text-left">Locations</Link><Link href="/services" onClick={() => setMenuOpen(false)} className="text-left">Services</Link><Link href="/about" onClick={() => setMenuOpen(false)} className="text-left">About</Link><Link href="/blog" onClick={() => setMenuOpen(false)} className="text-left">Insights</Link><Link href="/contact" onClick={() => setMenuOpen(false)} className="text-left text-accent">Book a Site Visit</Link></div>}
      </nav>
